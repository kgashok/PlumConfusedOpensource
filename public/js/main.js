...
function updateUserInfo(data) {
    ...
    if (data.authenticated && data.user) {
        ...
        if (isAuthorizedUser) {
            ...
            document.getElementById('imagineLink')?.classList.remove('hidden');
        } else {
...
async function showImaginePrompt() {
    const modal = document.getElementById('inspirationModal');
    const body = document.body;
    ...
    const modalContent = modal.querySelector('.prose');
    modalContent.innerHTML = `
        <h2 class="text-pink-600">✨ Imagine</h2>
        <p class="text-sm text-gray-500 mb-3">Generate an image using xAI's Aurora model (Grok Imagine)</p>
        <div class="bg-pink-50 p-4 rounded-lg mb-3">
            <textarea id="imaginePrompt" class="w-full bg-transparent border-none focus:outline-none text-gray-800 resize-none" rows="4" placeholder="Describe the image you want to generate..."></textarea>
        </div>
        <div class="flex items-center gap-3 mb-4">
            <label class="text-sm text-gray-600">Aspect ratio:</label>
            <select id="imagineAspectRatio" class="text-sm border rounded px-2 py-1">
        ...
        <button onclick="generateImagineImage()" class="bg-pink-500 hover:bg-pink-600 text-white font-bold py-2 px-4 rounded mb-4">
            Generate with xAI
        </button>
        <div id="imagineResult" class="hidden mt-4"></div>
    `;
}

async function generateImagineImage() {
    const prompt = document.getElementById('imaginePrompt').value;
    const aspect_ratio = document.getElementById('imagineAspectRatio').value;
    ...
    imagineResult.innerHTML = `<div class="text-gray-600 flex items-center gap-2"><svg class="animate-spin h-4 w-4 text-pink-500" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path></svg>Generating with xAI Aurora...</div>`;
    imagineResult.classList.remove('hidden');
    ...
    try {
        ...
        if (data.success) {
            imagineResult.innerHTML = `
                <img src="${data.imageUrl}" class="max-w-full rounded-lg shadow-lg" alt="Generated image" />
                <p class="text-xs text-gray-400 mt-2">Generated by xAI Aurora (Grok Imagine)</p>
            `;
        } else {
            imagineResult.innerHTML = `<div class="text-red-600">${data.error || 'Failed to generate image'}</div>`;
        }
    } catch (e) {
        imagineResult.innerHTML = `<div class="text-red-600">Failed to generate image</div>`;
    }
}

window.showImaginePrompt = showImaginePrompt;
window.generateImagineImage = generateImagineImage;
